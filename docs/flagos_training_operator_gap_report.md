...
`1.965290308`，MCM 从 `0.930582464` 降至 `0.658767521`，PDR 从 `1.308688283`
降至 `1.306522846`。首步约 `7075.2 ms`（包含惰性编译），去掉首步后的 steady
step p50 约 `212.1 ms`；相对首步约 `33.4x`，但这是小 shape smoke test，不是完整
...
- SToFM 训练桥：
  `https://github.com/priestess-bot/SToFM/tree/28e8794b0ceb93c5e7fa2fb1492bc2a3d3f6a42a`
- FlagGems 训练算子：
...
差异 `8.41e-8`，优化器状态 `1.19e-7`。此前多次独立进程复核的参数漂移上界为
