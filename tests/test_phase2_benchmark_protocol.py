...
def test_phase2_routes_keep_framework_operator_and_optimizer_attribution_separate():
    ...
    self_hosted = ROUTES["flagos_self_hosted_native_fused_v100_tuned"]
    assert self_hosted["gemm_backend"] == "self_hosted"
    assert self_hosted["gaussian_training_implementation"] == "native"
    assert self_hosted["attention_training_implementation"] == "native"
    assert self_hosted["optimizer"] == "flagos_fused"
