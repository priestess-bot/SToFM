...
def _config(backend, attention_backend=None):
    return SToFMConfig(
        num_hidden_layers=2,
        embedding_dim=32,
...
def test_default_flaggems_route_inherits_the_global_backend_for_attention():
    config = _config("flaggems")
...
def test_gaussian_adapter_matches_torch_fallback():
    ...
    optimized = GaussianModule(_config("flaggems")).to(device).eval()
...
def test_attention_adapter_preserves_pair_state_and_weights():
    ...
    optimized = MultiheadAttention(_config("torch", attention_backend="flaggems")).to(device).eval()
    optimized.load_state_dict(baseline.state_dict())
...
def test_encoder_can_skip_only_the_unused_final_pair_representation():
    device = torch.device("cuda")
    encoder = TransformerEncoder(_config("flaggems")).to(device).eval()
...
def test_end_to_end_adapter_preserves_input_gradients():
    ...
    optimized = SToFMModel(_config("flaggems")).to(device).eval()
    optimized.load_state_dict(baseline.state_dict())
